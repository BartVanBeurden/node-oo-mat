...
/**
	...
	@param {w1} Segment 1 Vertex 2
	@param {v2} Segment 2 Vertex 1
	@param {out} Segment 2 Vertex 2 such that 
		(v1, w1) perpendicular to (v2, w2) and
		(v1, w1) contains w2
**/
...
/**
	Compute shortest distance (squared) between line and vertex
	@param {v} Line Vertex 1
	@param {w} Line Vertex 2
	@param {p} Vertex to be tested
**/
Line2D.distance2ToPoint = 
Line2D.distance2ToVertex = (function() {
	var q = Vector2D.create();
	
	return function(v, w, p) {
		Line2D.perpendicular(v, w, p, q);
		return Vector2D.distance2(p, q);
	};
})();

/**
	Compute shortest distance between line and vertex
	@param {v} Line Vertex 1
	@param {w} Line Vertex 2
	@param {p} Vertex to be tested
**/
Line2D.distanceToPoint = 
Line2D.distanceToVertex = function(v, w, p) {
	return Math.sqrt(Line2D.distance2ToVertex(v, w, p));
};

/**
	Compute shortest distance (squared) between line segment and vertex
	@param {v} Segment Vertex 1
	@param {w} Segment Vertex 2
	@param {p} Vertex to be tested
**/
Line2D.segmentDistance2ToPoint =
Line2D.segmentDistance2ToVertex = (function() {
	var q = Vector2D.create();
	
	return function(v, w, p) {
		Line2D.perpendicular(v, w, p, q);
		if (Line2D.segmentContainsVertex(v, w, q)) {
			return Vector2D.distance2(p, q);
		} else {
			var d1 = Vector2D.distance2(v, p);
			var d2 = Vector2D.distance2(w, p);
			return d1 < d2 ? d1 : d2;
		}
	};
})();
...
/**
	Compute shortest distance between line segment and vertex
	@param {v} Segment Vertex 1
	@param {w} Segment Vertex 2
	@param {p} Vertex to be test
**/
Line2D.segmentDistanceToPoint = 
Line2D.segmentDistanceToVertex = function(v, w, p) {
	return Math.sqrt(Line2D.segmentDistance2ToVertex(v, w, p));
};
